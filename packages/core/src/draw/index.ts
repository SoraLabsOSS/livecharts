import type { LiveChartPalette, ChartLayout, LiveChartPoint, Momentum, ReferenceLine, OrderbookData, DegenOptions, CandlePoint } from '../types'
...
export interface MultiSeriesDrawOptions {
  ...
  primaryPalette: LiveChartPalette
}
...
export function drawMultiFrame(
  ...
  const scrubX = opts.scrubAmount > 0.05 ? opts.hoverX : null
  const allPts: { pts: [number, number][]; palette: LiveChartPalette; label?: string; alpha: number }[] = []
