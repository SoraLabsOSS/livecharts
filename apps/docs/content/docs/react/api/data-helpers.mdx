---
title: Data helpers
description: pushTick, createWalker, and aggregateCandles from livecharts/data
---

```ts
import { pushTick, createWalker, aggregateCandles } from "livecharts/data";
```
...
```ts
// Prefer a functional updater so async handlers never read stale `data`
setData((prev) => pushTick(prev, price, { keepSecs: 120 }).data);
setValue(price);
```

`time` defaults to `Date.now() / 1000` (unix **seconds**).
