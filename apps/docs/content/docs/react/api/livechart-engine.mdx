...
Build `palette` with [`resolveTheme`](/docs/react/api/theme).
...
For normal React apps prefer [`<LiveChart />`](/docs/react/api/livechart) or [`useLiveChartEngine`](/docs/react/api/use-live-chart-engine).
