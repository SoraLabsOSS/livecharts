...
With candle data present, toggling `mode` between `"line"` and `"candle"` runs a built-in morph (close-price line ↔ OHLC bodies). See [Candlestick](/docs/react/guides/candlestick).
