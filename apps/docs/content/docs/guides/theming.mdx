...
Pass **any CSS color** LiveCharts can parse (`#rgb`, `#rrggbb`, `rgb()`, `rgba()`). The library derives the full [`LiveChartPalette`](/docs/api/types#livechartpalette) via `resolveTheme`.
...
Each series carries its own `color`. If you omit colors, [`SERIES_COLORS`](/docs/api/theme) provides a default rotation.
